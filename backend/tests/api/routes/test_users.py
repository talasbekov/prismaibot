...
def test_get_users_superuser_me(
    ...
    r = legacy_client.get(
        f"{settings.API_V1_STR}/users/me", headers=legacy_superuser_token_headers
    )
    current_user = r.json()
    assert current_user
...
def test_get_users_normal_user_me(
    ...
    r = legacy_client.get(
        f"{settings.API_V1_STR}/users/me", headers=legacy_normal_user_token_headers
    )
    current_user = r.json()
    assert current_user
    assert current_user["is_active"] is True
...
def test_get_existing_user_current_user(legacy_client: TestClient, db: Session) -> None:
    username = random_email()
    password = random_lower_string()
    user_in = UserCreate(email=username, password=password)
    user = crud.create_user(session=db, user_create=user_in)
...
def test_retrieve_users(
    ...
    r = legacy_client.get(
        f"{settings.API_V1_STR}/users/", headers=legacy_superuser_token_headers
    )
    all_users = r.json()
...
def test_register_user(legacy_client: TestClient, db: Session) -> None:
    username = random_email()
    password = random_lower_string()
...
def test_register_user_already_exists_error(legacy_client: TestClient) -> None:
    password = random_lower_string()
    full_name = random_lower_string()
    data = {
...
def test_delete_user_me(legacy_client: TestClient, db: Session) -> None:
    username = random_email()
    password = random_lower_string()
    user_in = UserCreate(email=username, password=password)
    user = crud.create_user(session=db, user_create=user_in)
